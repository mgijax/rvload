...
def runSanityChecks ():
    ...
    for s in list(allStanzasDict.keys()):
        ...
        if prefix != 'RV' or len(suffix) != 7  :
            ...
            try:
                ...
                invalidIDList.append('%s - stanza with invalid primary ID' % id)
        hasAltId = 'alt_id' in currentStanzaDict
        ...
        if id == 'RV:0000000':
            continue
        # check synonymTypes
        hasSyn = 'synonym' in currentStanzaDict
        if not hasSyn:
    ...
    if len(invalidIDList) > 0:
        ...
        fpSanityRpt.write('Incorrectly formatted RV IDs %s%s'  % \
            (CRT, CRT))
        fpSanityRpt.write(CRT.join(invalidIDList))
    ...
    if len(invalidSynList) > 0:
        ...
        fpSanityRpt.write(CRT.join(invalidSynList))
        fpSanityRpt.write('%s%s' % ( CRT, CRT))
